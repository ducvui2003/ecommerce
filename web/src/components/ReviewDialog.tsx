...
import { orderApi, useGetReviewOfOrderItemQuery } from '@/features/order/order.api';
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
  ...
  const onSubmit = async (body: SendReviewReqType) => {
    ...
    try {
      ...
      if (result.hasOwnProperty('data')) {
        dispatch(orderApi.util.invalidateTags([{ type: 'OrderItem', id: item.id }]));
        setOpen(false);
        form.reset();
                      ...
                      <AlertDialog>
                        ...
                        <AlertDialogContent>
                          <AlertDialogHeader>
                            ...
                            <AlertDialogDescription>
                              Một khi đã chỉnh sửa thì không thể thao tác hành động này thêm
                              lần nào nữa. Nếu bạn đã chắc chắn hãy nhấn nút "Đồng ý"
                            </AlertDialogDescription>
                          </AlertDialogHeader>
                          <AlertDialogFooter>
